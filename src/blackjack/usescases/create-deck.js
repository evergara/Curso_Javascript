...
export const createdDeck = (typesOfCards, specialTypesOfCard) => {
  if (!typesOfCards || typesOfCards.length === 0) throw new Error("Type of card is mandatory and must to array of string");
...
const createdDeckSpecial = (decks, typesOfCards, specialTypesOfCards) => {
  if (!decks || decks.length === 0) throw new Error("The decks is mandatory and must to array of string");
  if (!typesOfCards || typesOfCards.length === 0) throw new Error("Type of card is mandatory and must to array of string");
  if (!specialTypesOfCards || specialTypesOfCards.length === 0) throw new Error("Type of special cards is mandatory and must to array of string");
